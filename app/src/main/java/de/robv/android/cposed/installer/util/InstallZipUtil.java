...
import de.robv.android.cposed.installer.BuildConfig;
import de.robv.android.cposed.installer.R;
import de.robv.android.cposed.installer.XposedApp;
import de.robv.android.cposed.installer.installation.FlashCallback;
...
public final class InstallZipUtil {
    ...
    public static ZipCheckResult checkZip(ZipFile zip) {
        ...
        ZipEntry xposedPropEntry = zip.getEntry("system/cposed.prop");
        if (xposedPropEntry != null) {
            try {
                result.mXposedProp = parseXposedProp(zip.getInputStream(xposedPropEntry));
            } catch (IOException e) {
                Log.e(XposedApp.TAG, "Failed to read system/cposed.prop from " + zip.getName(), e);
            }
        }
